        ...
        try {
            ...
            const response = await fetch('https://api.deepseek.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    top_p: 0.9,
                    max_tokens: 8192,
                    stream: false  // Explicitly disable streaming to get complete JSON response
                }),
            ...
            // Check content-type to ensure we're not receiving a stream
            const contentType = response.headers.get('content-type') || '';
            if (contentType.includes('text/event-stream') || contentType.includes('text/plain')) {
                clearTimeout(timeoutId);
                this.logger.error(`Received stream response instead of JSON. Content-Type: ${contentType}`);
                throw new Error('DeepSeek API returned stream response instead of JSON. This may indicate a configuration issue.');
            }

            const responseText = await response.text();
            this.logger.log(`DeepSeek API response received, length: ${responseText.length} characters`);
            
            let deepseekResponse;
            try {
                deepseekResponse = JSON.parse(responseText);
            } catch (parseError) {
                clearTimeout(timeoutId);
                this.logger.error(`Failed to parse DeepSeek response as JSON. Response preview: ${responseText.substring(0, 500)}`);
                throw new Error(`Failed to parse DeepSeek API response: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
            }
            ...
            if (!generatedText) {
                clearTimeout(timeoutId);
                this.logger.error(`No content in DeepSeek response. Full response: ${JSON.stringify(deepseekResponse).substring(0, 500)}`);
