<template>
    <button
        ...
        class="fixed top-2 left-2 sm:top-4 sm:left-4 z-50 p-1.5 sm:p-2 bg-white/80 dark:bg-neutral-800/80 backdrop-blur-sm rounded-md shadow-lg focus:outline-none focus:ring-2 focus:ring-red-600 cursor-pointer"
        aria-label="Menu de navegação"
        :aria-expanded="sidebarOpen ? 'true' : 'false'"
        aria-controls="main-menu"
    >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 text-neutral-700 dark:text-neutral-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
    ...
    <aside
        class="fixed inset-y-0 left-0 w-60 sm:w-64 border-r border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950 z-20 transform transition-transform duration-300 ease-in-out"
        :class="isLargeScreen ? 'translate-x-0' : (sidebarOpen ? 'translate-x-0' : '-translate-x-full')"
        id="main-menu"
        role="navigation"
    ...
    >
        <div class="h-full flex flex-col">
            <div class="p-4 border-b border-neutral-200 dark:border-neutral-800" v-if="settings">
                ...
                <div class="mt-3 sm:mt-4 flex items-center justify-center gap-1.5 sm:gap-2">
                    <a v-if="settings['blog.facebook']" :href="`https://facebook.com/${settings['blog.facebook']}`" target="_blank" rel="noopener noreferrer" title="Facebook"
                        class="flex items-center justify-center p-1.5 sm:p-2 text-neutral-600 hover:text-red-600 dark:text-neutral-400 dark:hover:text-red-500 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-red-500"
                        aria-label="Facebook">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    ...
                    <a v-if="settings['blog.twitter']" :href="`https://twitter.com/${settings['blog.twitter']}`" target="_blank" rel="noopener noreferrer" title="Twitter"
                        class="flex items-center justify-center p-1.5 sm:p-2 text-neutral-600 hover:text-red-600 dark:text-neutral-400 dark:hover:text-red-500 rounded-md transition-colors">
                ...
                <div class="p-1.5 sm:p-2 m-auto items-center justify-center flex">
                    <div v-if="isLoggedIn" class="flex items-center space-x-2 sm:space-x-3">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center mt-1 text-center">
                                <a href="/member/profile" class="text-xs sm:text-sm text-red-500 hover:underline focus:outline-none focus:ring-2 focus:ring-red-500 rounded">Profile</a>
                                <span class="text-neutral-400 dark:text-neutral-500 mx-1 sm:mx-2">|</span>
                                <button @click="logout" class="text-xs sm:text-sm text-red-500 hover:underline focus:outline-none focus:ring-2 focus:ring-red-500 rounded">Sign Out</button>
                            </div>
                        </div>
                    </div>
                    <div v-else class="flex items-center justify-center">
                        <div class="flex space-x-1.5 sm:space-x-2">
                            <a href="/member/login" class="text-xs sm:text-sm text-red-500 hover:underline focus:outline-none focus:ring-2 focus:ring-red-500 rounded">Sign In</a>
                            <span class="text-neutral-400 dark:text-neutral-600">|</span>
                            <a href="/member/register" class="text-xs sm:text-sm text-neutral-700 dark:text-neutral-300 hover:text-red-500 dark:hover:text-red-500 hover:underline focus:outline-none focus:ring-2 focus:ring-red-500 rounded">Register</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-3 sm:p-4">
                <button
                    ...
                    class="w-full flex items-center p-2.5 sm:p-3 mb-4 sm:mb-6 bg-neutral-200 dark:bg-neutral-800 hover:bg-neutral-300 dark:hover:bg-neutral-700 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-neutral-50 dark:focus:ring-offset-neutral-950"
                    aria-label="Abrir busca"
                    aria-haspopup="dialog"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-2 sm:mr-3 text-neutral-500 dark:text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    <span class="text-sm sm:text-base text-neutral-700 dark:text-neutral-300">Search</span>
                </button>
    ...
    <div v-if="searchModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="search-modal" role="dialog" aria-modal="true">
        <div class="flex items-start justify-center min-h-screen pt-2 sm:pt-4 px-2 sm:px-4 pb-20 text-center sm:block sm:p-0">
            <!-- Background overlay -->
            ...
            <!-- Modal panel -->
            <div class="inline-block align-bottom bg-white dark:bg-neutral-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full w-full max-w-[95vw] sm:max-w-2xl relative z-10">
                <div class="px-3 sm:px-4 pt-4 sm:pt-5 pb-3 sm:pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="w-full">
                            <!-- Search header with close button -->
                            <div class="flex justify-between items-center mb-3 sm:mb-4">
                                <h3 class="text-base sm:text-lg leading-6 font-medium text-neutral-900 dark:text-white" id="modal-title">
                                    Search
                                </h3>
                                <button @click="closeSearchModal" class="text-neutral-400 hover:text-neutral-500 focus:outline-none p-1">
    ...
    <div v-if="isLoggedIn && !isLargeScreen" class="fixed bottom-4 right-4 z-40">
        <button
...
const loadRecentPosts = async () => {
    ...
    try {
        const response = await blogAPI.posts.getAll({ limit: 5 });
